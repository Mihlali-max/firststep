...
function TemplateCanva1({ cv, opts }:{ cv:CV; opts:Opts }) {
  ...
  return (
    <div style={{ width:'210mm', minHeight:'297mm', fontFamily:'"Helvetica Neue", Arial, sans-serif', background:'white', fontSize:'9px' }}>
      <div style={{ background:c+'33', display:'flex', alignItems:'flex-end', minHeight:'80px', position:'relative' }}>
        ...
        <div style={{ paddingLeft:24,paddingBottom:16,paddingTop:16,flex:1 }}>
          ...
          {p.title && <div style={{ fontSize:13,color:'#555',marginTop:4 }}>{p.title}</div>}
        </div>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'72mm 1fr', minHeight:'calc(297mm - 80px)', paddingTop:28 }}>
        <div style={{ padding:'0 20px 24px 28px' }}>
          <div style={{ background:c+'22',borderRadius:14,padding:'16px 14px',marginBottom:20,marginTop:8 }}>
            {opts.showPhone&&p.phone&&<div style={{ display:'flex',gap:8,marginBottom:10,fontSize:9,color:'#333' }}>📞 {p.phone}</div>}
            {p.email&&<div style={{ display:'flex',gap:8,marginBottom:10,fontSize:9,color:'#333',wordBreak:'break-all' }}>✉ {p.email}</div>}
            {opts.showLoc&&p.location&&<div style={{ display:'flex',gap:8,fontSize:9,color:'#333' }}>📍 {p.location}</div>}
            {opts.showLinkedin&&p.linkedin&&<div style={{ display:'flex',gap:8,marginTop:10,fontSize:9,color:'#333',wordBreak:'break-all' }}>🔗 {p.linkedin}</div>}
          </div>
          {cv.edu.some(e=>e.school)&&<div style={{ marginBottom:20 }}>
            ...
            {cv.edu.filter(e=>e.school).map((e,i)=><div key={i} style={{ marginBottom:10 }}>
              {e.year&&<div style={{ fontSize:8.5,color:'#777',marginBottom:2 }}>{e.year}</div>}
              <div style={{ fontSize:9.5,fontWeight:700,color:'#1a1a1a' }}>{e.qualification}</div>
              <div style={{ fontSize:8.5,color:'#444' }}>{e.school}</div>
            </div>)}
          </div>}
          {cv.skills.length>0&&<div style={{ marginBottom:20 }}>
            <div style={{ fontSize:11,fontWeight:800,letterSpacing:'1.5px',textTransform:'uppercase',color:'#1a1a1a',marginBottom:6 }}>SKILLS</div>
            <div style={{ height:1,background:'#ccc',marginBottom:10 }}/>
            {cv.skills.map(s=><div key={s} style={{ display:'flex',alignItems:'center',gap:7,marginBottom:5,fontSize:9 }}>
              <div style={{ width:5,height:5,borderRadius:'50%',background:'#888',flexShrink:0 }}/>{s}
            </div>)}
          </div>}
          {opts.showLanguages&&langs.length>0&&<div>
            ...
            <div style={{ height:1,background:'#ccc',marginBottom:10 }}/>
            {langs.map(l=><div key={l} style={{ fontSize:9.5,color:'#333',marginBottom:5 }}>{l}</div>)}
          </div>}
        </div>
        <div style={{ padding:'0 28px 24px 20px',borderLeft:'1px solid #eee' }}>
          ...
          {cv.exp.some(e=>e.title)&&<div style={{ marginBottom:20 }}>
            ...
            {cv.exp.filter(e=>e.title).map((e,i)=><div key={i} style={{ marginBottom:14 }}>
              {(e.start||e.end)&&<div style={{ fontSize:9,fontWeight:700,color:c==='#8FA3B1'?'#7a8f9e':c,marginBottom:2 }}>{[e.start,e.end].filter(Boolean).join(' – ')}{e.volunteer?' (Volunteer)':''}</div>}
              <div style={{ fontSize:11,fontWeight:700,color:'#1a1a1a',marginBottom:5 }}>{e.title}{e.org?` — ${e.org}`:''}</div>
              {e.bullets&&e.bullets.split('\n').filter(b=>b.trim()).map((b,j)=><div key={j} style={{ display:'flex',alignItems:'flex-start',gap:6,marginBottom:3 }}>
    ...
    <div style={{ display:'flex',width:'210mm',minHeight:'297mm',fontFamily:'"Helvetica Neue",Arial,sans-serif',fontSize:'9px',background:'white' }}>
      <div style={{ width:'70mm',background:c,color:'white',padding:'28px 18px',display:'flex',flexDirection:'column',gap:16,flexShrink:0 }}>
        <div style={{ textAlign:'center',paddingBottom:16,borderBottom:'1px solid rgba(255,255,255,0.2)' }}>
          ...
          <div style={{ fontSize:14,fontWeight:800,lineHeight:1.2 }}>{p.name||'Your Name'}</div>
        ...
        <div>
          ...
          {p.email&&<div style={{ display:'flex',gap:6,marginBottom:5,fontSize:8.5,wordBreak:'break-all' }}>✉ {p.email}</div>}
          {opts.showPhone&&p.phone&&<div style={{ display:'flex',gap:6,marginBottom:5,fontSize:8.5 }}>☏ {p.phone}</div>}
          {opts.showLoc&&p.location&&<div style={{ display:'flex',gap:6,marginBottom:5,fontSize:8.5 }}>⊕ {p.location}</div>}
        </div>
        {cv.edu.some(e=>e.school)&&<div>
          ...
          {cv.edu.filter(e=>e.school).map((e,i)=><div key={i} style={{ marginBottom:8 }}>
            <div style={{ fontSize:9,fontWeight:700 }}>{e.qualification}</div>
        ...
        {cv.exp.some(e=>e.title)&&<div>
          ...
          {cv.exp.filter(e=>e.title).map((e,i)=><div key={i} style={{ marginBottom:12,paddingLeft:10,borderLeft:`3px solid ${c}` }}>
            ...
            {e.bullets&&e.bullets.split('\n').filter(b=>b.trim()).map((b,j)=><div key={j} style={{ display:'flex',gap:5,marginBottom:2.5 }}>
              <span style={{ fontSize:9,color:'#666',flexShrink:0 }}>•</span>
              <span style={{ fontSize:9,color:'#444',lineHeight:1.65 }}>{b.trim().replace(/^[-•]\s*/,'')}</span>
            </div>)}
          </div>)}
        </div>}
  ...
  return (
    <div style={{ width:'210mm',minHeight:'297mm',fontFamily:'"Helvetica Neue",Arial,sans-serif',fontSize:'9px',background:'white',padding:'28px 32px' }}>
      <div style={{ display:'flex',alignItems:'center',gap:20,marginBottom:20,paddingBottom:18,borderBottom:`3px solid ${c}` }}>
        {opts.showPhoto&&p.photo&&<img src={p.photo} style={{ width:80,height:80,borderRadius:'50%',objectFit:'cover',border:`2px solid ${c}`,flexShrink:0 }}/>}
        <div style={{ flex:1 }}>
          <div style={{ fontSize:26,fontWeight:900,color:'#111',textTransform:'uppercase' }}>{p.name||'YOUR NAME'}</div>
          {p.title&&<div style={{ fontSize:11,color:c,fontWeight:600,marginTop:4 }}>{p.title}</div>}
  ...
  return (
    <div style={{ width:'210mm',minHeight:'297mm',fontFamily:'"Helvetica Neue",Arial,sans-serif',fontSize:'9px',background:'white' }}>
      <div style={{ background:c,padding:'24px 32px',display:'flex',alignItems:'center',gap:20,position:'relative',overflow:'hidden' }}>
        ...
        {opts.showPhoto&&p.photo
          ?<img src={p.photo} style={{ width:80,height:80,borderRadius:'50%',objectFit:'cover',border:'3px solid rgba(255,255,255,0.5)',flexShrink:0 }}/>
        ...
        <div style={{ color:'white',position:'relative' }}>
          <div style={{ fontSize:24,fontWeight:900,textTransform:'uppercase' }}>{p.name||'YOUR NAME'}</div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'72mm 1fr' }}>
  ...
  return (
    <div style={{ width:'210mm',minHeight:'297mm',fontFamily:'Georgia,"Times New Roman",serif',fontSize:'9px',background:'#FDFCFB' }}>
      <div style={{ padding:'28px 36px 0',textAlign:'center' }}>
        ...
        {opts.showPhoto&&p.photo&&<img src={p.photo} style={{ width:76,height:76,borderRadius:'50%',objectFit:'cover',border:`2px solid ${c}`,margin:'0 auto 10px',display:'block' }}/>}
        <div style={{ fontSize:26,fontWeight:400,letterSpacing:'0.12em',textTransform:'uppercase',color:'#111' }}>{p.name||'YOUR NAME'}</div>
      ...
      <div style={{ padding:'4px 36px 28px',display:'grid',gridTemplateColumns:'60mm 1fr',gap:24 }}>
        <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
          {cv.edu.some(e=>e.school)&&<div>
            <div style={{ fontSize:9,letterSpacing:'3px',textTransform:'uppercase',color:c,marginBottom:8,fontFamily:'sans-serif' }}>Education</div>
            {cv.edu.filter(e=>e.school).map((e,i)=><div key={i} style={{ marginBottom:8 }}>
          ...
          {cv.skills.length>0&&<div>
            <div style={{ fontSize:9,letterSpacing:'3px',textTransform:'uppercase',color:c,marginBottom:8,fontFamily:'sans-serif' }}>Skills</div>
          ...
          {opts.showLanguages&&langs.length>0&&<div>
            <div style={{ fontSize:9,letterSpacing:'3px',textTransform:'uppercase',color:c,marginBottom:8,fontFamily:'sans-serif' }}>Languages</div>
            {langs.map(l=><div key={l} style={{ fontSize:9.5,color:'#444',fontFamily:'sans-serif',marginBottom:4 }}>{l}</div>)}
          </div>}
          {opts.showRefs&&cv.refs.some(r=>r.name)&&<div>
            <div style={{ fontSize:9,letterSpacing:'3px',textTransform:'uppercase',color:c,marginBottom:8,fontFamily:'sans-serif' }}>References</div>
        ...
        <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
          {cv.exp.some(e=>e.title)&&<div>
            <div style={{ fontSize:9,letterSpacing:'3px',textTransform:'uppercase',color:c,marginBottom:8,fontFamily:'sans-serif' }}>Experience</div>
...
const DUMMY:CV = {
  pi:{ name:'Thabo Nkosi', title:'Customer Service Agent', email:'thabo.nkosi@email.com', phone:'071 234 5678', location:'Johannesburg, Gauteng', summary:'Dedicated professional with strong communication skills. Quick learner who takes pride in delivering quality work and contributing positively to every team.', linkedin:'', photo:'', languages:'English, Zulu, Sotho' },
  edu:[{ school:'Soweto Secondary School', qualification:'National Senior Certificate', year:'2021' }],
  skills:['Customer service','Communication','MS Office','Teamwork','Problem solving','Cash handling','Data entry','Telephone etiquette'],
  exp:[
    { title:'Customer Service Rep', org:'Vodacom', start:'Mar 2022', end:'Present', bullets:'Handled 80+ customer calls daily.\nAchieved 95% customer satisfaction score.\nTrained 3 new team members on CRM systems.', volunteer:false },
    { title:'Sales Assistant', org:'Edgars', start:'Jan 2021', end:'Feb 2022', bullets:'Assisted customers with purchases.\nProcessed cash and card payments.\nMaintained store presentation.', volunteer:false },
  ],
  refs:[{ name:'Ms Lindiwe Dube', relation:'Team Leader, Vodacom', contact:'011 000 1234' }, { name:'Mr Sipho Khumalo', relation:'Store Manager, Edgars', contact:'011 000 5678' }],
}
...
function TemplateCard({ id, name, desc, color, selected, onSelect }:{ id:string; name:string; desc:string; color:string; selected:boolean; onSelect:()=>void }) {
  ...
  return (
    <button onClick={onSelect} className={clsx('relative border-2 rounded-2xl overflow-hidden text-left transition-all group hover:shadow-lg',selected?'border-[#F5A623] shadow-[0_0_0_3px_rgba(245,166,35,0.2)]':'border-black/8 hover:border-[#F5A623]/50')}>
      <div className="w-full overflow-hidden bg-white" style={{ height:230 }}>
        <div style={{ transform:'scale(0.28)', transformOrigin:'top left', width:'357%', pointerEvents:'none', userSelect:'none' }}>
      ...
      <div className="rounded-2xl border border-black/8 overflow-hidden">
        <div className="p-4 flex items-center gap-4 bg-white">
          ...
          <a href="https://www.canva.com/resumes/templates/" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1.5 text-xs font-bold px-4 py-2 rounded-xl text-white flex-shrink-0 hover:opacity-85"
        ...
        <div className="bg-[#F7F3EB] border-t border-black/6 px-4 py-3">
          <div className="flex items-center gap-3 overflow-x-auto" style={{scrollbarWidth:'none'}}>
            {[{n:'1',t:'Open Canva',d:'Click above'},{n:'2',t:'Pick template',d:'Resume section'},{n:'3',t:'Fill your info',d:'Edit details'},{n:'4',t:'Share & link',d:'Paste below'}].map((s,i,arr)=>(
              <div key={s.n} className="flex items-center gap-2 flex-shrink-0">
                ...
                {i < arr.length-1 && <div className="text-black/20 mx-1">→</div>}
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white border-t border-black/6 p-4">
          ...
          <div className="flex gap-2">
            <input className="flex-1 bg-[#F7F3EB] border border-black/10 text-[#1A1A0F] placeholder:text-black/25 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#F5A623]/60 transition-all"
              placeholder="https://www.canva.com/design/..." value={canvaUrl} onChange={e=>setCanvaUrl(e.target.value)}/>
            <button onClick={saveUrl} className="px-4 py-2.5 rounded-xl text-sm font-bold text-white hover:opacity-85 flex-shrink-0" style={{background:'linear-gradient(135deg,#7D2AE8,#00C4CC)'}}>
              {saved ? '✓ Saved' : 'Link CV'}
            </button>
          </div>
          {canvaUrl.includes('canva.com') && (
          ...
          <p className="text-[11px] text-black/25 mt-1.5">In Canva: Share → "Share link" → Copy link</p>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 mt-3">
        {[{icon:'🎨',t:'100s of templates',d:'Pro designs'},{icon:'📱',t:'Mobile friendly',d:'Design on phone'},{icon:'💾',t:'Free PDF',d:'Download free'}].map(f=>(
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <F label="Full name *"><input className={inp} placeholder="e.g. Thabo Nkosi" value={cv.pi.name} onChange={e=>setCv(c=>({...c,pi:{...c.pi,name:e.target.value}}))}/></F>
                      <F label="Job title"><input className={inp} placeholder="e.g. Customer Service Agent" value={cv.pi.title} onChange={e=>setCv(c=>({...c,pi:{...c.pi,title:e.target.value}}))}/></F>
                    </div>
              ...
              {step===2&&(
                <div>
                  ...
                  <div className="space-y-3">
                    {cv.edu.map((e,i)=>(
                      <div key={i} className="border border-black/8 rounded-2xl p-4 bg-[#FAFAFA] relative">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                          <F label="School"><input className={inp} placeholder="Soweto Secondary School" value={e.school} onChange={ev=>{const ed=[...cv.edu];ed[i].school=ev.target.value;setCv(c=>({...c,edu:ed}))}}/></F>
                          <F label="Qualification"><input className={inp} placeholder="National Senior Certificate" value={e.qualification} onChange={ev=>{const ed=[...cv.edu];ed[i].qualification=ev.target.value;setCv(c=>({...c,edu:ed}))}}/></F>
                        </div>
                    ...
                    <button onClick={()=>setCv(c=>({...c,edu:[...c.edu,{school:'',qualification:'',year:''}]}))} className="flex items-center gap-2 text-sm font-semibold text-[#C47D0A] hover:text-[#F5A623]">
              ...
              {step===4&&(
                <div>
                  ...
                  <div className="bg-[#F5A623]/10 border border-[#F5A623]/20 rounded-2xl px-4 py-3.5 mb-5 mt-2">
                    ...
                    <p className="text-sm text-black/45">Put each responsibility on a new line — they'll appear as proper bullet points on your CV.</p>
                  </div>
                  <div className="space-y-3">
                    {cv.exp.map((e,i)=>(
                      <div key={i} className="border border-black/8 rounded-2xl p-4 bg-[#FAFAFA] relative">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                          <F label="Job title"><input className={inp} placeholder="Customer Service Rep" value={e.title} onChange={ev=>{const ex=[...cv.exp];ex[i].title=ev.target.value;setCv(c=>({...c,exp:ex}))}}/></F>
                          <F label="Company"><input className={inp} placeholder="Vodacom" value={e.org} onChange={ev=>{const ex=[...cv.exp];ex[i].org=ev.target.value;setCv(c=>({...c,exp:ex}))}}/></F>
                        </div>
                        <div className="grid grid-cols-2 gap-3 mb-3">
                          <F label="Start"><input className={inp} placeholder="Jan 2022" value={e.start} onChange={ev=>{const ex=[...cv.exp];ex[i].start=ev.target.value;setCv(c=>({...c,exp:ex}))}}/></F>
                        ...
                        <F label="What you did (one per line)">
                          <textarea className={inp+' resize-none'} rows={4} placeholder={"Handled customer calls daily.\nResolved billing queries.\nMet monthly satisfaction targets."} value={e.bullets} onChange={ev=>{const ex=[...cv.exp];ex[i].bullets=ev.target.value;setCv(c=>({...c,exp:ex}))}}/>
                        </F>
                    ...
                    <button onClick={()=>setCv(c=>({...c,exp:[...c.exp,{title:'',org:'',start:'',end:'',bullets:'',volunteer:false}]}))} className="flex items-center gap-2 text-sm font-semibold text-[#C47D0A] hover:text-[#F5A623]">
                  ...
                  <div className="space-y-3">
                    {cv.refs.map((r,i)=>(
                      <div key={i} className="border border-black/8 rounded-2xl p-4 bg-[#FAFAFA] relative">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                          <F label="Full name"><input className={inp} placeholder="Ms Lindiwe Dube" value={r.name} onChange={e=>{const rf=[...cv.refs];rf[i].name=e.target.value;setCv(c=>({...c,refs:rf}))}}/></F>
                          <F label="Relationship"><input className={inp} placeholder="Team Leader" value={r.relation} onChange={e=>{const rf=[...cv.refs];rf[i].relation=e.target.value;setCv(c=>({...c,refs:rf}))}}/></F>
                        </div>
                        <F label="Phone"><input className={inp} placeholder="011 000 1234" value={r.contact} onChange={e=>{const rf=[...cv.refs];rf[i].contact=e.target.value;setCv(c=>({...c,refs:rf}))}}/></F>
                      </div>
                    ))}
                    <button onClick={()=>setCv(c=>({...c,refs:[...c.refs,{name:'',relation:'',contact:''}]}))} className="flex items-center gap-2 text-sm font-semibold text-[#C47D0A] hover:text-[#F5A623]">
