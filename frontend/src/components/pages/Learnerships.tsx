...
export default function Learnerships() {
  ...
  useEffect(() => {
    let cancelled = false
    const q = search.trim() ? search.trim() : QUERIES[activeQ].q
    const timer = setTimeout(async () => {
      setLoading(true)
      try {
        const cleanQ = q + (q === 'learnership' ? ' -senior -manager -director' : ''); let url = `${import.meta.env.VITE_API_BASE || ''}/api/jobs?q=${encodeURIComponent(cleanQ)}&page=${page}&results_per_page=20`
        ...
        if (!cancelled) { setError(true); setJobs(FALLBACK); setTotal(FALLBACK.length) }
      } finally {
    ...
    <div className="min-h-[calc(100vh-68px)] bg-[#F7F3EB]">
      ...
      <div className="bg-[#1A1A0F] px-4 md:px-12 py-10 md:py-14">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative mb-5">
            ...
            <input value={search} onChange={e=>{ setSearch(e.target.value); setPage(1) }} placeholder="Search jobs, companies, skills…"
              className="w-full bg-white/15 border border-white/30 text-white placeholder:text-white/40 rounded-2xl pl-11 pr-11 py-4 caret-white text-sm outline-none focus:bg-white/12 focus:border-white/30 transition-all"/>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          <button onClick={()=>{ setPage(1); setSearch(s=>s+' '); setTimeout(()=>setSearch(s=>s.trim()),10) }} className="flex items-center gap-1.5 text-sm text-black/40 hover:text-black transition-colors px-3 py-2.5 rounded-xl hover:bg-white">
                        ...
                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-black/6 gap-3 flex-wrap">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <a href={`https://wa.me/?text=${encodeURIComponent('🎯 ' + job.title + ' at ' + job.company + '\n📍 ' + job.location + '\n\nApply: ' + job.apply_url)}`}
                              ...
                              className="flex items-center justify-center bg-[#25D366] text-white px-3.5 py-2.5 rounded-xl hover:bg-[#1ebe5d] transition-colors flex-shrink-0"
