...
echo "📤 Adding CV upload + AI extraction..."

# Add the upload route to App.tsx
sed -i "s|import CVBuilder from './components/pages/CVBuilder'|import CVBuilder from './components/pages/CVBuilder'\nimport CVUpload from './components/pages/CVUpload'|" src/App.tsx
sed -i "s|<Route path=\"/cv\" element={<CVBuilder />} />|<Route path=\"/cv\" element={<CVBuilder />} />\n          <Route path=\"/cv/upload\" element={<CVUpload />} />|" src/App.tsx

cat > src/components/pages/CVUpload.tsx << 'EOF'
...
echo "✅ CV Upload page done! Now adding backend extract endpoint..."
