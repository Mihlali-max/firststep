...
cat > src/components/pages/Home.tsx << 'EOF'
...
const FEATURES = [
  { icon:'⭐', title:'AI Career Coach', desc:'Chat with an AI that understands the SA job market. Interview prep, job advice in English, isiXhosa, or Zulu — voice enabled.', to:'/coach' },
  ...
  { icon:'🔍', title:'Learnership Finder', desc:'Browse SETA learnerships, YES Programme, and entry-level jobs near you — filtered for no-experience roles.', to:'/learnerships' },
  { icon:'📚', title:'Skills Micro-courses', desc:'Short lessons on cover letters, interview skills, and workplace basics — built for first-time job seekers.', to:'/' },
  { icon:'✅', title:'LAP Programme Checker', desc:'Search active Labour Activation Programme opportunities from the Dept. of Employment & Labour — live from official sources.', to:'/lap' },
  { icon:'👥', title:'Community & Support', desc:'Connect with others on the same journey. Share advice, celebrate wins, and get peer support across SA.', to:'/' },
]
const TESTIMONIALS = [
  ...
  { quote:'"The AI coach helped me practice my interview answers. When I sat in the real thing I already knew what to say."', name:'Thabo S.', role:'Soweto, Johannesburg' },
  { quote:'"I found a LAP learnership paying R3 500/month, five minutes from my house. I didn\'t know learnerships existed before this app."', name:'Zanele D.', role:"Mitchell's Plain, Cape Town" },
]
...
export default function Home() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setPVis(false)
      setTimeout(() => {
        const n = (pIdx+1)%PHRASES.length; setPIdx(n); setPTxt(PHRASES[n]); setPVis(true)
      }, 650)
    }, 3200)
  ...
  useEffect(() => {
    ...
    const dots = Array.from({length:55},()=>({x:Math.random()*W,y:Math.random()*H,r:Math.random()*1.6+0.3,vx:(Math.random()-.5)*.3,vy:(Math.random()-.5)*.3}))
    ...
    const draw = () => {
      ...
      for(let i=0;i<dots.length;i++) for(let j=i+1;j<dots.length;j++) {
        ...
        if(d<130){ctx.beginPath();ctx.strokeStyle=`rgba(245,166,35,${0.15*(1-d/130)})`;ctx.lineWidth=0.5;ctx.moveTo(dots[i].x,dots[i].y);ctx.lineTo(dots[j].x,dots[j].y);ctx.stroke()}
      }
      dots.forEach(d=>{ctx.beginPath();ctx.arc(d.x,d.y,d.r,0,Math.PI*2);ctx.fillStyle='rgba(245,166,35,0.55)';ctx.fill();d.x+=d.vx;d.y+=d.vy;if(d.x<0||d.x>W)d.vx*=-1;if(d.y<0||d.y>H)d.vy*=-1})
  ...
  useEffect(() => {
    const hero=heroRef.current,photo=photoRef.current; if(!hero||!photo) return
    photo.style.transition='transform 0.4s ease'
    const move=(e:MouseEvent)=>{const r=hero.getBoundingClientRect();const x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;photo.style.transform=`scale(1.05) translate(${x*-14}px,${y*-10}px)`}
  ...
  return (
    <>
      {/* HERO */}
      <section ref={heroRef} className="relative min-h-[calc(100vh-68px)] grid grid-cols-1 lg:grid-cols-2 items-center gap-12 px-8 md:px-12 py-20 overflow-hidden"
        ...
        <div className="absolute right-[-80px] top-[-80px] w-[560px] h-[560px] rounded-full pointer-events-none z-0"
          style={{background:'radial-gradient(circle,rgba(245,166,35,0.18) 0%,rgba(245,166,35,0.04) 50%,transparent 75%)'}}/>
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 text-[#F5A623] text-[11px] font-medium tracking-[3px] uppercase bg-[#F5A623]/12 border border-[#F5A623]/25 px-3 py-1.5 rounded-full mb-5"
            ...
            <span className="w-1.5 h-1.5 rounded-full bg-[#F5A623]"/>For South African Youth
          </div>
          <h1 className="font-display font-bold leading-[1.05] tracking-tight text-[#FFFDF7] mb-5"
            style={{fontSize:'clamp(2.8rem,5vw,4.6rem)',animation:'fadeup 1s ease .5s both'}}>
            Your first job<br/>
            <span className="pl-12">starts{' '}
              <em className="not-italic text-[#F5A623]" style={{fontSize:'0.75em',display:'inline-block',opacity:pVis?1:0,transform:pVis?'translateY(0)':'translateY(-14px)',transition:'opacity .65s cubic-bezier(.16,1,.3,1),transform .65s cubic-bezier(.16,1,.3,1)'}}>
                {pTxt}
              </em>
            </span>
          </h1>
          <p className="text-[#FFFDF7]/65 font-light text-base leading-relaxed max-w-md mb-9" style={{animation:'fadeup .8s ease .7s both'}}>
            Build your CV, find learnerships, check government programmes, and get coached by AI — completely free.
          </p>
          <div className="flex gap-3 flex-wrap mb-14" style={{animation:'fadeup .8s ease .9s both'}}>
            <Link to="/register" className="btn-amber flex items-center gap-2">Get started free <ArrowRight size={16}/></Link>
            <Link to="/lap" className="btn-outline !text-[#FFFDF7]/85 !border-[#FFFDF7]/25 hover:!border-[#FFFDF7]/60 hover:!bg-[#FFFDF7]/5">Check LAP programmes →</Link>
          </div>
          <div className="flex gap-7 border-t border-[#FFFDF7]/10 pt-7" style={{animation:'fadeup .8s ease 1.1s both'}}>
        ...
        <div className="relative z-10 hidden lg:block">
          <img ref={photoRef}
            ...
            className="w-full h-[520px] object-cover object-top rounded-2xl shadow-[0_24px_64px_rgba(0,0,0,0.45)]"/>
          ...
          <div className="absolute -top-5 -right-5 bg-[#1A1A0F]/85 border border-[#F5A623]/35 rounded-2xl px-5 py-4 backdrop-blur-xl min-w-[160px]"
            style={{animation:'floatcard 5s ease-in-out 2s infinite'}}>
        ...
        <div className="flex" style={{animation:'marquee 30s linear infinite',width:'max-content'}}>
          {[0,1].map(i=>(
            <div key={i} className="flex items-center">
              {['Build your CV','Find learnerships','Check LAP programmes','AI career coach','Interview prep','YES Programme','SETA learnerships','No experience needed'].map(item=>(
                <span key={item} className="flex items-center">
                  <span className="text-[#F5A623]/70 text-xs font-medium tracking-[1.5px] uppercase px-7">{item}</span>
      ...
      <section className="bg-[#F7F3EB] py-20 px-8 md:px-12 border-b border-[#1A1A0F]/10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map(f=>(
              <Link key={f.title} to={f.to} className="card reveal group">
                <div className="w-12 h-12 rounded-xl bg-[#F5A623]/10 flex items-center justify-center text-2xl mb-5">{f.icon}</div>
                ...
                <span className="text-sm font-medium text-[#C47D0A] flex items-center gap-1 group-hover:gap-2 transition-all">Learn more <ChevronRight size={14}/></span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#1A1A0F] py-20 px-8 md:px-12">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="reveal">
            ...
            <Link to="/lap" className="inline-flex items-center gap-2 text-[#F5A623] border border-[#F5A623]/30 hover:bg-[#F5A623]/10 hover:border-[#F5A623] transition-all px-6 py-3 rounded-xl text-sm font-medium">Search LAP programmes →</Link>
          </div>
          <div className="flex flex-col gap-4 reveal">
            {[['9','Provinces covered — search by your location'],['Free','Training costs covered by UIF contributions'],['Live','AI searches official sources in real time']].map(([n,l])=>(
              <div key={n} className="flex items-center gap-5 bg-[#FFFDF7]/5 border border-[#FFFDF7]/8 rounded-xl px-6 py-5">
      ...
      <section className="py-20 px-8 md:px-12 border-b border-[#1A1A0F]/10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map(t=>(
              <div key={t.name} className="reveal border-t-2 border-[#1A1A0F]/10 hover:border-[#F5A623]/40 transition-colors pt-6">
                <div className="flex gap-0.5 mb-4">{[0,1,2,3,4].map(i=><Star key={i} size={14} className="fill-[#F5A623] text-[#F5A623]"/>)}</div>
      ...
      <section className="bg-[#F5A623] py-20 px-8 md:px-12 flex flex-wrap items-center justify-between gap-8">
        ...
        <Link to="/register" className="btn-ink flex items-center gap-2 whitespace-nowrap">Create your free profile <ArrowRight size={16}/></Link>
      </section>
