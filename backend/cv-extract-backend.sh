...
echo "🤖 Adding CV extract endpoint..."

cat > app/api/cv_extract.py << 'EOF'
...
if "cv_extract" not in content:
    ...
    with open("app/main.py", "w") as f:
        f.write(content)
    print("✅ cv_extract router registered in main.py")
else:
    print("ℹ️  cv_extract already registered")
PYEOF

echo "✅ Backend extract endpoint done!"
echo ""
echo "Now add ANTHROPIC_API_KEY to backend/.env:"
echo "  ANTHROPIC_API_KEY=your-api-key"
echo ""
echo "Then restart the backend:"
echo "  fuser -k 8000/tcp && uvicorn app.main:app --reload --port 8000"
